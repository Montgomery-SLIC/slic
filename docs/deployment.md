...
| `slic_django_production.service` | systemd unit for production |
| `slic_django_staging.service` | systemd unit for staging |

## Environment

Production reads from `/etc/slic_django.env` (or the path set in the systemd unit's `EnvironmentFile`). All the variables in [setup.md](setup.md#environment-variables) apply, plus:
...
sudo systemctl restart slic_django_production
```

## Gunicorn configuration

Both configs set `workers = (2 x CPU cores) + 1`, `timeout = 120`, and log to `/var/log/slic_django/`. The log directory must exist and be writable by the service user before starting.

| | Staging (`gunicorn.conf.py`) | Production (`gunicorn_prod.conf.py`) |
|-|------------------------------|--------------------------------------|
| Bind | `127.0.0.1:9294` | `127.0.0.1:9295` |
| Access log | `/var/log/slic_django/access.log` | `/var/log/slic_django/access_prod.log` |
| Error log | `/var/log/slic_django/error.log` | `/var/log/slic_django/error_prod.log` |
| Log level | `info` | `warning` |

Django itself also logs warnings and above to `/var/log/slic_django/django.log` (production only, configured in `slic/settings/production.py`).
...
```bash
pg_dump slic_django_production | gzip > /backups/slic_$(date +%Y%m%d).sql.gz
```
